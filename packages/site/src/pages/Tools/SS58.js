                ...
                <div style={{width:'400px',height:'540px',background:'white',textAlign:'center',boxShadow:'0 2px 10px 0 rgba(0,0,0,.05)'}}>
                    <div style={{marginTop:'20px'}}>
                        <div style={{position:'absolute',marginTop:'50px',marginLeft:'40px'}}>
                            ...
                            <TextArea value={searchValue} onChange={handleSearchValue} rows={2} style={{marginTop:'15px',width:'320px', height:'74px', resize:'none',color:'#606266',boxShadow:'0 2px 10px 0 rgba(0,0,0,.05)'}}/>
                            {notFound ? <div style={{color:'red',fontSize:'12px',marginTop:'10px'}}>转换失败，未找到此账号或公钥</div> : null}
                            <Button onClick={handleTransform} block style={{marginTop:'220px',width:'320px',height:'40px',background:'#3f3f3f',color:'white'}}>转换</Button>
                        </div>
                    </div>
                </div>
            </div>
