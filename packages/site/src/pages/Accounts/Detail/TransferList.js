      ...
      <Table
          ...
          dataSource={transfers.map(item => {
            return {
              ...
              direction: item.data[0] === address ? (<div style={{
                    display:'flex',
                    justifyContent:'center',background:'#D32F2F',color:'white'}}>
                    {$t('transfer_out')}</div>) :
                  (<div style={{
                    display:'flex',
                    justifyContent:'center',background:'#4CAF50',color:'white'}}>{$t('transfer_in')}</div>)
            }
          })}
          columns={[
            {
