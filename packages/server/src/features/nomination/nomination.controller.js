...
const { getExtrinsicCollection,getValidatorsCollection,getEventCollection } = require('../../services/mongo')
...
class NominationController {
    ...
    async getNomination(ctx) {
        ...
        for (let i=0; i< nominationList.length;i++) {
            const currentValidator = validatorList.find(element => element.account == nominationList[i].args.target);
            const data = await eventCol.find({'extrinsicHash':nominationList[i].hash})
            nominationWithValidtorInfo.push({
                ...
                nomination: nominationList[i].name == 'bond' ? nominationList[i].args.value : 0,
                name: nominationList[i].name,
                revocations: nominationList[i].name === 'unbond' ? nominationList[i].args.value: 0,
